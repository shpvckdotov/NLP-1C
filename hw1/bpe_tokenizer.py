...
class BPETokenizer:
    def __init__(self, vocab_size = 30000):
        ...
        self.idx_to_token.update({idx: token for token, idx in self.special_tokens.items()})
    ...
    def encode(self, text, return_attention_mask):
        ...
        input_ids = [self.token_to_idx.get(token, self.special_tokens[self.unk_token]) for token in tokens]
    ...
    def decode(self, input_ids):
        tokens = [self.idx_to_token.get(token_id, self.unk_token) for token_id in input_ids]
